...
//                 <p className="text-secondary m-0 p-0">{product.price}</p>
...
function ProductList() {
  ...
  const calculatePricePerUnit = (price: number, unit: number): number => {
    return unit > 0 ? price / unit : 0;
  };

  return (
    <div className="container-fluid list d-flex flex-wrap justify-content-center bg-body-tertiary p-2">
      <section className="d-flex flex-wrap gap-4 list">
        {busy && (
          <div className="loading-indicator">
            <Spinner animation="border" role="status">
          ...
          <div key={product.id} className="card mt-3" style={{ width: "18rem" }}>
            <div className="card-body">
              ...
              <section>
                ...
                <p className="text-secondary m-0 p-0">Price: ${product.price} per unit</p>
              </section>
            </div>
          </div>
        ))}
